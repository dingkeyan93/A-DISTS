    ...
    def compute_prob(self, feats):
        ...
        for k in range(len(feats) - 1, -1, -1):
            try:
                ...
                x_var = F.conv2d(pad(feats[k]**2), self.windows[k], stride=1, padding=0, groups=feats[k].shape[1]) - x_mean**2
                h, w = x_mean.shape[2], x_mean.shape[3]
                gamma = torch.mean(x_var / (x_mean + c0), dim=1, keepdim=True)
                ps = 1 / (1 + torch.exp(-(gamma - gamma.mean(dim=(2, 3), keepdim=True)) / (gamma.std(dim=(2, 3), keepdim=True) + c0)))
    ...
    def forward(self, x, y, as_loss=True):
        ...
        for k in range(len(self.chns) - 1, -1, -1):
            ...
            try:
                ...
                x_var = F.conv2d(pad(feat_x**2), self.windows[k], stride=1, padding=0, groups=self.chns[k]) - x_mean**2
                y_var = F.conv2d(pad(feat_y**2), self.windows[k], stride=1, padding=0, groups=self.chns[k]) - y_mean**2
